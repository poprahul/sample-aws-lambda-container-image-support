...
```bash
my-sam-function$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
my-sam-function$ sam local start-api
my-sam-function$ curl http://localhost:3000/
```
...
```bash
my-sam-function$ sam logs -n HelloWorldFunction --stack-name my-sam-function --tail
```
...
```bash
my-sam-function$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name my-sam-function
```
